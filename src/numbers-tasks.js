...
function toNumber(value, def) {
  if (Number.isNaN(Number(value)) || value == null) return def;
  return value;
}
...
function isPowerOfTwo(num) {
  if (2 ** Math.floor(Math.log2(num)) === num) return true;
  return false;
}
...
function getCountOfOddNumbers(number) {
  let count = 0;
  for (let i = 1; i <= Math.abs(number); i += 2) count += 1;
  return count;
}
